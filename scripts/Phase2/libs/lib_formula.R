...
if (!Sys.getenv("DEV_LIB_FORMULA", unset=FALSE)=="TRUE") {
    ...
    get_base_names<- function(names, isNum=FALSE){
        ...
        for (x in 1:length(names)){
            # for quadratics
            value <- trimws(names[[x]])
            if (startsWith(value, "I")==TRUE){
                item <- substring(value, 3, nchar(value)-3)
            ...
            # for intercept
            if (startsWith(value, "(")==TRUE) next

            # for interections
            pos <- regexpr(':', value)
            if (pos>0){
                item <- substring(value, 1, pos-1)
                if (is.na(match(item, new_names))==TRUE){
                    new_names <- c(new_names, item)
                }
                item <- substring(value, pos+1)
                if (is.na(match(item, new_names))==TRUE){
                    new_names <- c(new_names, item)
                }
            }
            else{
                if (is.na(match(value, new_names))==TRUE){
                    new_names <- c(new_names, value)
                }
            }
        }
        if (isNum==TRUE){
