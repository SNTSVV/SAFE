...
#ENV$PARAMS <- c("results/test", "results/test/_phase2/_samples/sample_0005.md", "results/test/_phase2/workdata.csv", 10, 20, 0.0597)
...
sampled_data <- generate_samples_by_distance(TASK_INFO, fx, yID, XID, 100, 20)

# write results
options(scipen=999)  # prevent to print scientific notation (exponential notation, 1e+5)
write.table(samples, file=sampleFile, append=FALSE, sep=",", dec= ".", row.names = FALSE, col.names = TRUE)
options(scipen=0)    # revert print option
################################################################################
