...
if (!Sys.getenv("DEV_LIB_EVALUATE", unset=FALSE)=="TRUE") {
    ...
    #############################################
    calculate_metrics<-function(model, evalData, borderP){

        Positive_value <- "0"  # no deadline miss
        predicted <- predict(model, evalData, type="response")
        predicted <- ifelse(as.numeric(predicted) < borderP, 0, 1)
        answers <- ifelse(evalData$result==0, 0, 1)

        # basic values
        cdf <-ConfusionDF(y_pred=predicted, y_true=answers)
    ...
    kfoldCV <- function(model, data, K){
        ...
        for(i in 1:K){
            ...
            eval.result <- calculate_metrics(md_test, testPart, borderP)
            eval.result <- data.frame(Iter=i, eval.result)
